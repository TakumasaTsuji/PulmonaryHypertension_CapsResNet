...
# -*- coding: utf-8 -*-
"""
Created on Fri Sep 18 16:46:21 2020
...
from lib.MakeDatasets import MakeDatasets, split_cross_validation_dataset, save_dataset, load_tokushima_dataset, load_image_preprocessing
...
def run(args):
    ...
    train_dataset, validation_dataset, test_dataset = split_cross_validation_dataset(
        dataset, k=args.k, k_folds=10, seed=args.seed)
    
    
    train_image_dataset = load_image_preprocessing(train_dataset, size)
    validation_image_dataset = load_image_preprocessing(validation_dataset, size)
    test_image_dataset = load_image_preprocessing(test_dataset, size)
    ...
    save_dataset(train_dataset, os.path.join(save_train_path, "train_dataset.csv"))
    save_dataset(validation_dataset, os.path.join(save_train_path, "validation_dataset.csv"))
    save_dataset(test_dataset, os.path.join(save_train_path, "test_dataset.csv"))
...
def train_model(args, model, train_iter, validation_iter, save_train_path):
    ...
    # Setup an optimizer
    optimizer = chainer.optimizers.Adam(1e-3 * args.lr, beta1=0.5) 
