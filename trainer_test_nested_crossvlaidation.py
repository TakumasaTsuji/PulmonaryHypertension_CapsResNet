...
from models import CapsuleResNet_512



def load_model(args):
    if args.ModelName == "CapsuleResNet_512":
        model = CapsuleResNet_512.CapsResNet(cls=args.cls)
        size = 512
...
def run(args):
    ...
    #Train
    #save_train_path = os.path.join(args.out, str(args.lr))
    save_train_path = args.out
    os.makedirs(save_train_path, exist_ok=True)
    save_dataset(sub_train_dataset, os.path.join(save_train_path, "train_dataset.csv"))
    save_dataset(sub_validation_dataset, os.path.join(save_train_path, "validation_dataset.csv"))
    save_dataset(sub_test_dataset, os.path.join(save_train_path, "test_dataset.csv"))
    ...
    sum_pred, sum_label, classification_result, name_list = test_model(args, best_model_path, test, test_dataset, save_test_path)
...
def test_model(args, best_model_path, test, test_dataset, save_test_path):
