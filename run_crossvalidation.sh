...
SAVEBASEPATH="/data/HDD1/Tokushima_Project/result/PAm/revise/RSNA/TrasnferLearning/2class/crossvalidation"
...
do
    ...
    do
	#SAVEDIRPATH="${SAVEBASEPATH}/${MODEL}/${k}"
	SAVEMODELPATH="${SAVEBASEPATH}/${MODEL}/${step}/${k}"
	python trainer_test_crossvalidation.py -g ${GPU} -c ${CLS} -m ${MODEL} -pr ${MODELPATH} -b ${BATCHSIZE} -lr ${step} -e ${EPOCH} -o ${SAVEMODELPATH} -p ${PATHDIR} -csv ${CSVPATH} -k ${k} 
